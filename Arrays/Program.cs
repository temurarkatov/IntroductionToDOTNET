...
namespace Arrays
{
	internal class Program
	{
		static void Main(string[] args)
		{
			...
			Console.Write("Введите колличество элементов строки: ");
			int cols = Convert.ToInt32(Console.ReadLine());

			int[,] arr = new int[rows, cols];
			Console.WriteLine($"Колличество измерений: {arr.Rank}");
			Console.WriteLine($"Колличество элементов: {arr.Length}");
			...
			Console.WriteLine($"Минимальное значение элементов в массиве: {arr.Cast<int>().Min()}");
			Console.WriteLine($"Максимальное значение элементов в массиве: {arr.Cast<int>().Max()}"); 
#endif
			int[][] arr =
				{
				new int[]{ 0,1,1,2},
